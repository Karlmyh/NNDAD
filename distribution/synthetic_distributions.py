...
class TestDistribution(object):
    ...
    def testDistribution_2(self,dim):
        density1 = MultivariateNormalDistribution(mean=np.zeros(dim) + 0.7,cov=np.diag(np.ones(dim)*0.0025)) 
        density2 = MultivariateNormalDistribution(mean=np.zeros(dim) + 0.3,cov=np.diag(np.ones(dim)*0.01)) 
    ...
    def testDistribution_5(self,dim):
        density1 = MultivariateNormalDistribution(mean=np.zeros(1) + 0.7,cov=np.diag(np.ones(1)*0.0025)) 
        density2 = MultivariateNormalDistribution(mean=np.zeros(1) + 0.3,cov=np.diag(np.ones(1)*0.01)) 
        density_seq = [density1, density2]
        prob_seq = [0.4,0.6]
        densitymix = MixedDistribution(density_seq, prob_seq)
        
        
        densitymarginal = MarginalDistribution([densitymix for _ in range(dim)])
